#ifndef PARSE_WASM_PROGRAM_DEF_H
#define PARSE_WASM_PROGRAM_DEF_H
#include "wasm_base.h"
...
private:
	...
	{
		std::underlying_type_t<wasm_language_type> type;
	...
	struct table_def_t
	{
		...
		std::size_t initial_size;
		std::optional<std::size_t> maximum;
		std::underlying_type_t<wasm_language_type> element_type;
		std::vector<init_segment_t> init_segments;
	...
	using external_kind_t = std::underlying_type_t<external_kind>;

	struct import_t 
	{
		import_t(external_kind_t kind_, std::size_t index_):
			kind(kind_), index(index_), defined(false)
		{
	...
	{
		export_t(external_kind_t kind_, std::size_t index_):
	...
	struct module_mappings_t
	{
		...
		void map_index(external_kind_t kind, std::ptrdiff_t to_index, std::ptrdiff_t from_index = -1);
	};
	...
	std::size_t resolve_import(const std::string& name, external_kind_t kind);

	std::size_t add_export(const std::string& name, external_kind_t kind);
	
	std::size_t add_empty_definition(external_kind_t kind);
	
	template <class Def>
	std::ptrdiff_t add_function_definition(Def&& def, std::ptrdiff_t idx = -1)
	{
	...
	template <class Def>
	std::ptrdiff_t add_table_definition(Def&& def, std::ptrdiff_t idx = -1)
	{
	...
	template <class Def>
	std::ptrdiff_t add_linear_memory_definition(Def&& def, std::ptrdiff_t idx = -1)
	{
	...
	template <class Def>
	std::ptrdiff_t add_global_variable_definition(Def&& def, std::ptrdiff_t idx = -1)
	{
	...
	std::ptrdiff_t add_program_definition(Definition&& def, 
					...
					std::ptrdiff_t idx = -1)
	{
		...
		return idx < 0 ? static_cast<std::ptrdiff_t>(defs.size() - 1) : idx;
	}
	
	void resolve_initializers(module_mappings_t& mappings);
	void finalize_function_code(wasm_code_string_t& code, const module_mappings_t& mappings);
...
#endif /* PARSE_WASM_PROGRAM_DEF_H */
