...
wasm_function::wasm_function(const opcode_t* code_begin, const opcode_t* code_end, std::size_t sig, 
		std::size_t nlocals, std::size_t nparams, std::size_t nreturns):
	func_storage(FunctionStorage_New(code_begin, code_end, sig, nlocals, nparams, nreturns))
{
	if(not func_storage)
		throw std::bad_alloc();
}
...
std::size_t wasm_function::signature() const
{
...
std::size_t wasm_function::param_count() const
{
	return FunctionStorage_ParamCount(func_storage.get());
}
